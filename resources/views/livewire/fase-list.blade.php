                ...
                @foreach ($fases as $fase)
                    @if ($editable)
                        <tr>
                            ...
                            <td class="alin text-center">
                                ...
                                <button class="btn btn-primary btn-sm" data-toggle="modal"
                                    data-target="#faseModalEdit{{ $fase->id }}">
                                ...
                                @if ($fase->tipo !== 'Inscripcion' && $fase->tipo !== 'Finalizacion')
                                    <form id="FormularioEli" action="{{ route('fase.delete', $fase->id) }}"
                                        ...
                                        @csrf
                                        @method('DELETE')
                                        <button class="btn btn-danger btn-sm btn-eliminar" type="submit"><i
                        ...
                        <form action="{{ route('faseEdit', $fase->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            <div class="modal fade" id="faseModalEdit{{ $fase->id }}" tabindex="-1"
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <div class="form-group">
                                                ...
                                                <input type="text" class="form-control" id="nombre_fase"
                                                    name="nombre_fase" required
                                                    value="{{ old('nombre_fase', $fase->nombre_fase) }}">
                                            </div>

                                            <div class="form-group">
                                                <label for="descripcion_fase">Descripción de la
                                                    fase:</label>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select name="tipo" class="form-control" id="tipo" required
                                                    {{ $fase->tipo == 'Inscripcion' || $fase->tipo == 'Finalizacion' ? 'disabled' : '' }}>
                                                    <option value="General"
                                                        {{ $fase->tipo == 'General' ? 'selected' : '' }}>
                                                        General</option>
                                                    <option value="Calificacion"
                                                        {{ $fase->tipo == 'Calificacion' ? 'selected' : '' }}>
                                                        Calificación</option>
                                                    <option value="Inscripcion"
                                                        {{ $fase->tipo == 'Inscripcion' ? 'selected' : '' }}>
                                                        Inscripción</option>
                                                    <option value="Finalizacion"
                                                        {{ $fase->tipo == 'Finalizacion' ? 'selected' : '' }}>
                                                        Finalización</option>
                                                </select>
                                            </div>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <input type="datetime-local" class="form-control" id="fechaInicio"
                                                    ...
                                                    value="{{ old('fechaInicio', $fase->fechaInicio->format('Y-m-d\TH:i')) }}">
                                            </div>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <input type="datetime-local" class="form-control" id="fechaFin"
                                                    ...
                                                    value="{{ old('fechaFin', $fase->fechaFin->format('Y-m-d\TH:i')) }}">
                                            </div>
                                            <div class="d-flex flex-column">
                                                <button type="submit" id="botonfases"
...
<script>
    $(".btn-eliminar").on("click", function(e) {
        ...
        if (confirm("¿Está seguro de eliminar esta fase?")) {
            var form = $(".btn-eliminar").parents('form:first');
            form.submit();
        }
    });
</script>
